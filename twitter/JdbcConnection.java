...
//            userIdx = rs2.getInt(1);
    ...
    static int[] insertNoticeIdx(int userIdx, java.sql.Timestamp d) {
        ...
        int returnValue[] = new int[10]; // ?ïåÎ¶? 10Í∞úÍπåÏß?Îß?
        ...
        try {
            ...
            while (true) {
                ...
                String s1 = "insert into twitter.Notifications(userIdx,notificationDate,notification) "
                        ...
                        + d + "),'FOLLOW');"; // ?ûÖ?†•?êú ?ãúÍ∞ÑÏùÑ Í∏∞Ï? ?ù¥?õÑÎ°? ?ÉùÍ∏? ?åîÎ°úÏö∞ ?ïå?ûå
                stmt.executeQuery(s1);
                String s2 = "select last_insert_id();"; // ?ïåÎ¶? ?ãú?ûëÏß??†ê
                s1 = "insert into twitter.Notifications(userIdx,notificationDate,notification) "
                        ...
                        + "from Comment where Comment.createAt >" + d + ")),'COMMENT');"; // ?ûÖ?†•?êú ?ãúÍ∞ÑÏùÑ Í∏∞Ï? ?ù¥?õÑÎ°? ?ÉùÍ∏? comment
                // ?ïå?ûå
                stmt.executeQuery(s1);
    ...
    static String returnNotification(int noticeIdx) { // noticeidx?óê ÎßûÎäî notice ?Ç¥?ö©
        Connection conn = null;
        String returnValue = ""; // ?ù¥?õÑ Î¨∏Ï†ú?ûà?úºÎ©? nullÎ°? ?àò?†ïÍ∞??ä•
        ...
        try {
            ...
            while (true) {
                stmt = conn.createStatement();
                String s3 = "select content from Notifications where notificationIdx='" + noticeIdx + "'"; // postIdx?óê
                // ÎßûÎäî
                // postContent
                // ÎΩëÏïÑ?Ç¥?äî ÏøºÎ¶¨Î¨?
    ...
    static java.sql.Timestamp returnNoticeDate(int noticeIdx) {
        // noticeidx?óê ÎßûÎäî notice ?ãúÍ∞?
        Connection conn = null;
        java.sql.Timestamp returnValue = null; // timestamp?äî ?Ç¨?ö©?ûê?ãú?†ê?úºÎ°? Ï¢ãÏ? Î™ªÌïú ?ãúÍ∞ÑÍ∞í?ùÑ Ï∂úÎ†•?ï¥Ï§? //Í∑∏Îûò?Ñú ?ù¥?õÑ Î¨∏Ï†ú?óÜ?úºÎ©? Date?òï?ãù?úºÎ°? Î∞îÍøîÏ£ºÎäî Í±?
        // Ï∂îÏ≤ú!!!!!!!!
        ...
        try {
            ...
            while (true) {
                stmt = conn.createStatement();
                String s4 = "select content from Notifications where notificationIdx='" + noticeIdx + "'"; // noticeidx?óê
                // ÎßûÎäî
                // noticetime
    ...
    static String[] trendHash() {
        ...
        try {
            ...
            while (true) {
                stmt = conn.createStatement();
                String s1 = "select hash from (select hash,count(hash) from HashTag order by count(hash) desc) A"; // ?ÉÅ?úÑ
                // 10Í∞?
                // hash
                // Ï∞æÍ∏∞
    ...
    static int returnHashNumber(String hash) { // ?äπ?†ï Hash?óê ???ï¥?Ñú hashÍ∞??àòÎ•? return?ïò?äî class
        Connection conn = null;
        int returnValue = 0; // ?ù¥?õÑ Î¨∏Ï†ú?ûà?úºÎ©? nullÎ°? ?àò?†ïÍ∞??ä•
        ...
        try {
            ...
            while (true) {
                stmt = conn.createStatement();
                String s2 = "select count(hash) from HashTag where hash='" + hash + "'"; // hash?óê ÎßûÎäî Hash Í∞??àò ÎΩëÏïÑ?Ç¥?äî ÏøºÎ¶¨Î¨?
                rs = stmt.executeQuery(s2);
                if (rs.next()) {
    ...
    static int[] searchPostidx() {
        ...
        int[] searchResult = new int[100]; // Ï¥àÍ∏∞Í∞? 0
        ...
        try {
            ...
            while (true) {
                ...
                stmt = conn.createStatement();
                String s1 = "select distinct postIdx from POST where content LIKE '%" + searchContents + "%'"; // Í≤??Éâ ÏøºÎ¶¨Î¨?
                // ÏøºÎ¶¨Î¨? ?àò?†ï?òà?†ï
                rs = stmt.executeQuery(s1);
                int i = 0;
    ...
    static String returnUserID(int postIdx) { // postidx?óê ???ï¥?Ñú postUserIdÎ•? return?ïò?äî class
        Connection conn = null;
        String returnValue = ""; // ?ù¥?õÑ Î¨∏Ï†ú?ûà?úºÎ©? nullÎ°? ?àò?†ïÍ∞??ä•
        ...
        try {
            ...
            while (true) {
                stmt = conn.createStatement();
                String s2 = "select postUserIdx from POST where postIdx = '" + postIdx + "'"; // postIdx?óê ÎßûÎäî userid ÎΩëÏïÑ?Ç¥?äî
                // ÏøºÎ¶¨Î¨?
                rs = stmt.executeQuery(s2);
                if (rs.next()) {
                    returnValue = rs.getString(1);
    ...
    static String returnPostContent(int postIdx) { // postidx?óê ???ï¥?Ñú post ?Ç¥?ö©Î¨? return?ïò?äî class
        Connection conn = null;
        String returnValue = ""; // ?ù¥?õÑ Î¨∏Ï†ú?ûà?úºÎ©? nullÎ°? ?àò?†ïÍ∞??ä•
        ...
        try {
            ...
            while (true) {
                stmt = conn.createStatement();
                String s3 = "select content from Post where postUserIdx='" + postIdx + "'"; // postIdx?óê ÎßûÎäî postContent
                // ÎΩëÏïÑ?Ç¥?äî ÏøºÎ¶¨Î¨?
                rs = stmt.executeQuery(s3);
                if (rs.next()) {
                    returnValue = rs.getString(1);
