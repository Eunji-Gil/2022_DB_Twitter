...
public class OtherFollowing extends JFrame implements ActionListener {
	static int MyIdx;
	static int userIdx;
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					OtherFollowing window = new OtherFollowing();
	...
	public OtherFollowing(int userIdx, String userName, String userId, int followingCount,int MyIdx) {
		this.userIdx = userIdx;
		this.UserId = userId;
		this.UserName = userName;
		this.followingCount = (int) followingCount;
		this.followingCeilCount = (int) ((Math.ceil(followingCount / 5.0)) + 1) * 5;
		max_page = followingCount / 5;
		this.MyIdx = MyIdx;
	}

	public OtherFollowing() {
		try {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (bb.equals(profileButton)) {
			dispose();
			OtherProfile nam = new OtherProfile(userIdx,MyIdx);
			OtherProfile le = new OtherProfile(); // View Profile
			le.main(null);
		}
		if (bb.equals(next) && max_page > current_page) {
			count += 5;
			profileLable1.setIcon(profilePhoto(Following[count + 1][5]));
			profileLable2.setIcon(profilePhoto(Following[count + 1][5]));
			profileLable3.setIcon(profilePhoto(Following[count + 1][5]));
			profileLable4.setIcon(profilePhoto(Following[count + 1][5]));
			profileLable5.setIcon(profilePhoto(Following[count + 1][5]));
